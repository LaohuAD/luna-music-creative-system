...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Count characters with luna music sing-control rules."
    )
    src = parser.add_mutually_exclusive_group()
    src.add_argument("--text", help="Text to count directly")
    src.add_argument("--file", help="Path to a UTF-8 text file to count")
    parser.add_argument("--preset", choices=["sing-md"], help="Use a built-in extraction preset")
    ...
    print("notes=counts lyrics, tags, spaces, punctuation, tildes, parentheses, and newlines")
    return 0
